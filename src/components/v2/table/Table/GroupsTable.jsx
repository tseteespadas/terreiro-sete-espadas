...
export default function GroupsTable({
  ...
  const groupsData = groups
    ...
    .filter(({ group_name }) => {
      return group_name.toLowerCase().search(groupSearch.toLowerCase()) !== -1;
    })
    .sort((a, b) => {
      if (nameSortAsc !== null) {
        if (nameSortAsc) {
          ...
          return a.group_name.localeCompare(b.group_name) * -1;
        }
      }
  ...
  return (
    <TableContainer>
      ...
      <StyledGroupsTable>
        <caption>
          Tabela de Grupos de Usuários. Contém colunas Nome e Opções.
        </caption>
        <thead>
          <tr>
            <th>
          ...
          {groupsData.map((group) => {
            return (
              <tr key={group.group_id}>
                <td className="highlight">{group.group_name}</td>
                <td>
                  <UserActions>
                    <button
                    ...
                    <Link
                      to={`/grupos/${group.group_id}`}
                      title="Visualizar membros do grupo"
                    >
                      <FontAwesomeIcon
                        className="icon new"
